...
function App() {
  ...
  const [cart, setCart] = useState(() => {
    const salvat = localStorage.getItem('ecopet_cart');
  ...
  const loadUserData = async (uid) => {
    ...
    const { data: profData } = await supabase.from('profile').select('rol, puncte').eq('id', uid).single();
    if (profData) {
      setUserRole(profData.rol);
      setUserPoints(profData.puncte || 0);
    }

    const { data: wishData } = await supabase.from('lista_dorinte').select('produs_id').eq('utilizator_id', uid);
    if (wishData && wishData.length > 0) {
      const ids = wishData.map(w => w.produs_id);
      const { data: favProducts } = await supabase.from('produs').select('*').in('id', ids);
      if (favProducts) setWishlist(favProducts);
    } else {
      setWishlist([]);
    }
  };
